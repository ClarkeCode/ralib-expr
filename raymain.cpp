...
int main(void)
{
	...
	while (!WindowShouldClose()) { // Detect window close button or ESC key
		...
		BeginDrawing();
			...
			DrawText(TextFormat("AAA %f %f %f", SpreadV3(camera.position)), 10, 60, 20, BLACK);
			// DrawText(TextFormat("AAA %f %f %f", camera.position.x, camera.position.y, camera.position.z), 10, 60, 20, BLACK);
			DrawFPS(10, 10);
		EndDrawing();
		//----------------------------------------------------------------------------------
	}
